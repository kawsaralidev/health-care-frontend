...
const DoctorBooking = ({ doctorId }: DoctorBookingProps) => {
  const router = useRouter();

  const { data: me, isPending: mePending } = useAuthHooks.useGetMe();
    ...
    <>
      <div>
        {schedules.map((schedule) => (
          <div
            key={schedule.id}
            className="flex items-center gap-5 rounded-md border p-3"
          >
